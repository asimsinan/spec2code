...
export class SDDImplementTool {
  ...
  getToolDefinition(): Tool {
    return {
      name: 'sdd_implement',
      description: 'Provides atomic implementation instructions for a specific phase (1-4) with 76 total tasks. Phase 1: 27 tasks (Foundations), Phase 2: 22 tasks (Core), Phase 3: 17 tasks (UI+APIs), Phase 4: 10 tasks (Testing+Deployment). Each task includes mandatory verification, RED-GREEN-REFACTOR-SMOKE pattern, and proof requirements. MUST be called for implementation - returns complete instructions with anti-gaming enforcement.',
      inputSchema: {
        type: 'object',
        properties: {
          phase: {
            type: 'string',
            description: 'Phase number (1-4) to execute immediately: 1=Foundations & Data (27 tasks), 2=Core Implementation (22 tasks), 3=UI Development with Real APIs (17 tasks), 4=Full Integration & Verification (10 tasks)'
          }
        },
        required: []
      }
...
`;
  ...
  private generateDesignTaskInstructions(phaseTasks: any, platform?: string): string {
    ...
    return `
### 🎨 Design Tasks (${designTasks.length} items)
${taskDescriptions}
...
`;
  ...
  private generateTaskBasedInstructions(phaseTasks: any, testType: string, testCommands?: string, platform?: string): string {
    ...
    const relevantTasks = phaseTasks.tasks.filter((task: any) => {
      ...
      if (testType.toLowerCase() === 'design') {
        return task.id?.startsWith('DESIGN-') || 
               task.tddPhase?.toLowerCase().includes('design') ||
               task.subPhase?.toLowerCase().includes('design') ||
               task.title?.toLowerCase().includes('design');
      }
...
${this.generateTaskBasedInstructions(phaseTasks, 'Design', undefined, platform)}
...
- ${completionText}`;
  ...
  /**
   * Generate continuous execution policy for pre/post tasks
   */
...
- Only stop when ALL tasks are complete and verified`;
  ...
  private generatePreTaskInstructions(phaseTasks: any, platform?: string, language?: string): string {
    const preTasks = [
      ...
      {
        id: 'PRE-TASK-005',
        title: 'VERIFY Design System Setup',
        description: 'Validate Tailwind CSS is properly configured, no circular dependencies exist, and CSS compilation succeeds',
        verification: 'Check Tailwind imports, CSS compilation, and verify no circular dependencies'
      }
    ];
...
${this.getTaskContinuousExecutionPolicy()}
  ...
  private generatePostTaskInstructions(phaseTasks: any, platform?: string): string {
    const postTasks = [
      ...
      {
        id: 'POST-TASK-004',
        title: 'VERIFY Spec Compliance',
        description: 'Verify EVERY feature from specification is implemented with code evidence',
        verification: 'Check spec requirements against actual implementation, provide code references for each feature'
      },
      {
        id: 'POST-TASK-005',
        title: 'VERIFY Design System Quality',
        description: 'Confirm Tailwind CSS is working properly, no circular dependencies, and all components render correctly',
        verification: 'Test CSS compilation, verify Tailwind classes work, check for design system errors'
      }
    ];
